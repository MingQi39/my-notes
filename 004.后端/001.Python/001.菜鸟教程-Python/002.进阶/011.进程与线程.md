---
date: 2026-04-01 16:05
modified: 2026-04-01 17:38
---
...
**使用** `map` **方法批量提交任务（注意：** `map` **方法本身不阻塞，但读取其返回的生成器对象是阻塞的，并且得到结果的顺序，与任务分配的顺序是一致的）**

map 方法会把这一批任务提交到进程池里执行，它会立刻返回一个生成器，真正的阻塞发生在：生成器取结果时，如 list(result)
...
![[Pasted image 20260401173359.png|600]]

GIl 锁何时会被释放？ —— 主动释放（遇到 I/O 操作）、被迫释放（任务超时）

![[Pasted image 20260401173456.png|600]]

GIL 对比 lock/Rlock：

![[Pasted image 20260401173548.png|600]]
...
![[Pasted image 20260401173748.png]]
