---
date: 2026-03-24 15:38
modified: 2026-04-01 16:03
---
...
1. `read(size)` 中的 `size` 是可选参数。
	1. 若不传递`size`参数，表示：读取文件中所有的内容（注意内存占用！）。
	2. 若传递了`size`参数，表示：读取文件中指定个数的字符，或指定大小的字节。
...
1. `readline(size)` 中的 `size` 是可选参数。
	1. 若不传递`size`参数，表示：读取当前这一行所有的内容。
	2. 若传递了 `size` 参数，表示：表示读取当前行时，最多能读取的字符数，或字节数
	3. 注意：`size`不是行数。
...
1. `readlines(hint)` 中的 `hint` 是可选参数。
	1. 若不传递`hint`参数，表示：读取当前文件的所有行。
	2. 若传递了 `hint` 参数，表示：期望读取的【字符个数 或 字节数】（`hint` 不是行数）。
...
`os` 模块是 Python 标准库中的一个重要模块，它提供了与操作系统交互的功能。通过 `os` 模块，可以执行文件操作、目录操作、环境变量管理、进程管理等任务。

`os` 模块是跨平台的，这意味着可以在不同的操作系统（如 Windows、Linux、macOS）上使用相同的代码。
...
| 65  | [](https://www.runoob.com/python/os-write.html)[os.pardir()](https://www.runoob.com/python3/python3-os-pardir.html)获取当前目录的父目录，以字符串形式显示目录名。                                                               |
