---
date: 2026-03-24 14:22
modified: 2026-04-01 11:39
---
...
![[Pasted image 20260324142414.png|200]]

装饰器的语法使用 @decorator_name 来应用在函数或方法上。内置的装饰器，比如 @staticmethod 和 @classmethod，用于定义静态方法和类方法。
...
![[Pasted image 20260324143410.png|300]]

## 多个函数装饰器一起使用

核心：注意装饰顺序，距离函数最近的装饰器，会先工作。

例如下面代码：test 2 先装饰，test 1 再装饰。
...
def log_class(cls):
    ...
    class Wrapper:
        ...
        def display(self):
            print(f"调用 {cls.__name__}.display() 前") # 1. 调用 MyClass.display() 前
            self.wrapped.display()
            print(f"调用 {cls.__name__}.display() 后") # 3. 调用 MyClass.display() 后
...
class MyClass:
    def display(self):
        print("这是 MyClass 的 display 方法") # 2. 这是 MyClass 的 display 方法
...
class Database:
    def __init__(self):
        print("Database 初始化") # Database 初始化
