...
CSRF 攻击的核心是浏览器 [[004.cookie 与 session#cookie 的组成|根据规则]] 自动携带 cookie 的默认行为，所以完全可以不使用 cookie，而是用 [[005.JWT|token]] 和 [[004.浏览器离线存储|浏览器本地存储]] 实现身份认证
...
过去很常用，现在已经==发现漏洞==：当 URL 是 [[015.Base64|Base64]] 编码形式时，Base64 编码代码中的请求不会携带 Referer 请求头。
