...
![[Pasted image 20260703170912.png|600]]
...
- **问题**：如果不处理，随着对话进行，`state["messages"]` 会包含几千条消息。直接全部传给 LLM 会导致：1. 烧钱；2. 超过 128 k/8k 限制报错。
...
def invoke_with_trim(agent, user_input: str, config: dict):
    ...
    if existing_messages:
        ...
        trimmed_messages = trim_messages(
            ...
            allow_partial=False,                  # False 不允许部分消息，会尝试保留消息的完整性。如果无法在保持消息完整性的前提下将总 token 数裁剪到参数 max_tokens 以内，就会返回 空列表
