...
```python title=manage_conversation_history
...
- 主要差异点：
    ...
    2. 内置工具支持 - Web 搜索、文件搜索、计算机操作等
    3. 事件驱动架构 - 更可预测的流式响应
    4. 简化的 agentic 工作流 - 专为 AI Agent 设计

```python title=response_api
from openai import OpenAI
from dotenv import load_dotenv
load_dotenv(override=True)
