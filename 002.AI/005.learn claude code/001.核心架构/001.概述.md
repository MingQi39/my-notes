...
**第一级：代码补全**（如 Copilot）。这本质上是一个**单次预测问题**——模型不需要理解整个项目，不需要执行任何操作，只需要根据局部上下文生成合理的代码片段。

**第二级：IDE 聊天助手**（如 Cursor Chat、Copilot Chat）。但关键限制是：**模型不能执行操作**。它生成一个 diff，由用户决定是否 apply。如果 diff 有问题（比如依赖了一个不存在的函数），用户需要手动发现并反馈，模型无法自行验证。

**第三级：自主 Agent**（Claude Code）。模型不仅生成代码，还能**自主执行多步操作**。整个过程是一个**自主决策循环**——模型决定下一步做什么，执行后观察结果，再决定下一步。
...
**模型是循环中的决策者，而非人类**。人类设定目标（"给这个项目添加用户认证"）并审批危险操作（"确认执行 `npm install`？"），但在两次人类交互之间，模型自主决定读什么文件、改什么代码、执行什么命令。
...
1. **Snip**（零 API 成本）：移除对话历史中已经不再被引用的旧工具结果。例如，10 轮前的一次 `grep` 搜索结果可能有 50 KB，但模型早已不再关注它。Snip 用一个占位符替换这些内容，纯本地操作，不需要调用 API。（`query.ts:401-410`）
...
详见 [第 3 章：上下文工程](https://github.com/Windy3f3f3f3f/how-claude-code-works/blob/main/docs/03-context-engineering.md)。
...
## [1.4 源码目录结构](https://windy3f3f3f3f.github.io/how-claude-code-works/#/docs/01-overview?id=_14-%e6%ba%90%e7%a0%81%e7%9b%ae%e5%bd%95%e7%bb%93%e6%9e%84)

Claude Code 源码约 1,900 文件、512 K+ 行 TypeScript，目录结构如下：
...
## [1.5 数据流全景](https://windy3f3f3f3f.github.io/how-claude-code-works/#/docs/01-overview?id=_15-%e6%95%b0%e6%8d%ae%e6%b5%81%e5%85%a8%e6%99%af)

理解 Claude Code 的关键是理解**数据如何在各层之间流动**。下面是一次完整的用户交互的数据流：

![[数据流全景.excalidraw|800]]
...
### [关于性能：流式工具预执行](https://windy3f3f3f3f.github.io/how-claude-code-works/#/docs/01-overview?id=%e5%85%b3%e4%ba%8e%e6%80%a7%e8%83%bd%ef%bc%9a%e6%b5%81%e5%bc%8f%e5%b7%a5%e5%85%b7%e9%a2%84%e6%89%a7%e8%a1%8c)
...
### [关于错误恢复](https://windy3f3f3f3f.github.io/how-claude-code-works/#/docs/01-overview?id=%e5%85%b3%e4%ba%8e%e9%94%99%e8%af%af%e6%81%a2%e5%a4%8d)
...
## [1.6 启动流程](https://windy3f3f3f3f.github.io/how-claude-code-works/#/docs/01-overview?id=_16-%e5%90%af%e5%8a%a8%e6%b5%81%e7%a8%8b)

Claude Code 的启动经过精心优化，将大量工作并行化和延迟化。整个流程分为 9 个阶段，关键路径仅约 **235 ms**：

![[启动流程.excalidraw|500]]

### [为什么分 9 个阶段？](https://windy3f3f3f3f.github.io/how-claude-code-works/#/docs/01-overview?id=%e4%b8%ba%e4%bb%80%e4%b9%88%e5%88%86-9-%e4%b8%aa%e9%98%b6%e6%ae%b5%ef%bc%9f)
...
- **Phase 9 懒加载重依赖**：OpenTelemetry（~400 KB+）在用户完成 Trust Dialog 之后才加载，避免拖慢启动速度

## [1.7 架构总览](https://windy3f3f3f3f.github.io/how-claude-code-works/#/docs/01-overview?id=_17-%e6%9e%b6%e6%9e%84%e6%80%bb%e8%a7%88)
...
### [模块依赖规则](https://windy3f3f3f3f.github.io/how-claude-code-works/#/docs/01-overview?id=%e6%a8%a1%e5%9d%97%e4%be%9d%e8%b5%96%e8%a7%84%e5%88%99)
...
## [1.8 代码规模参考](https://windy3f3f3f3f.github.io/how-claude-code-works/#/docs/01-overview?id=_18-%e4%bb%a3%e7%a0%81%e8%a7%84%e6%a8%a1%e5%8f%82%e8%80%83)
